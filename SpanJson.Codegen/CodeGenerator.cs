...
namespace SpanJson.Codegen
{
    public static class CodeGenerator<TSymbol, TResolver> where TSymbol : struct where TResolver : class, IJsonFormatterResolver<TSymbol, TResolver>, new()
    {
        ...
        public static string Generate(Type[] dataTypes)
        {
            ...
            sb.AppendLine("using SpanJson.Shared.Models;");
            ...
            var alreadyDone = new HashSet<Type>();
            while (queue.Count > 0)
            {
                ...
                sb.AppendLine(
                    $"public sealed class {current.Name}{SymbolName}Formatter : BaseGeneratedFormatter<{currentFullName},{typeof(TSymbol).Name},ExcludeNullsOriginalCaseResolver<{typeof(TSymbol).Name}>>");
                sb.AppendLine("{");
                ...
                sb.AppendLine($"public override {currentFullName} Deserialize(ref JsonReader<{typeof(TSymbol).Name}> reader)");
                sb.AppendLine("{");
                GenerateDeserializer(sb, current, objectDescription, queue);
                sb.AppendLine("}");
                sb.AppendLine(
                    $"public override void Serialize(ref JsonWriter<{typeof(TSymbol).Name}> writer, {current.FullName.Replace('+', '.')} value)");
                sb.AppendLine("{");
        ...
        private static void GenerateSerializerNameFields(StringBuilder sb, Type current, JsonObjectDescription objectDescription)
        {
            foreach (var memberInfo in objectDescription.Members)
            {
                if (memberInfo.CanWrite)
                {
                    ...
                    {
                        var bytes = FormatUtf8Bytes("\"{memberInfo.Name}\":");
                        sb.AppendLine($"private byte[] _{memberInfo.MemberName}Name => {bytes};");
                    }
                }
            }
        }

        private static string FormatUtf8Bytes(string encodedName)
        {
            var bytes = Encoding.UTF8.GetBytes(encodedName);
            return $"new byte[]{{{string.Join(", ", bytes.Select(a => "0x"+a.ToString("X2")))}}}";
        }
        ...
        private static void GenerateWriteValue(StringBuilder sb, JsonMemberInfo memberInfo, int counter, Queue<Type> queue)
        {
            ...
            sb.AppendLine($"{GetPropertyFormatter(memberInfo, queue)}.Serialize(ref writer, value.{memberInfo.MemberName});");
        ...
        private static void GenerateDeserializer(StringBuilder sb, Type current, JsonObjectDescription objectDescription, Queue<Type> queue)
        {
            ...
            sb.AppendLine("{");
            sb.AppendLine($"var name = reader.Read{SymbolName}EscapedNameSpan();");
        ...
        private static void GenerateIfStatements(ICollection<JsonMemberInfo> memberInfos, StringBuilder sb, Queue<Type> queue, int index)
        {
            ...
            {
                (var readMethod, var keySuffix) = GetReadMethod(group.Key.intType);
                ...
                if (group.Count() == 1) // need to check remaining values too 
                {
                    ...
                    while (length < memberInfo.Name.Length)
                    {
                        ...
                        (var readSubMethod, var keySubSuffix) = GetReadMethod(subKey.intType);
        ...
        {
            var remaining = memberName.Length - index;
            if (remaining >= 8)
            {
