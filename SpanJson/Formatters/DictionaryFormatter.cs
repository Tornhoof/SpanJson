...
namespace SpanJson.Formatters
{
    ...
    public TDictionary? Deserialize(ref JsonReader<TSymbol> reader)
    {
        ...
        while (!reader.TryReadIsEndObjectOrValueSeparator(ref count))
        {
            ...
            AssignKvpFunctor(result, key,
                value); // No shared interface for IReadOnlyDictionary and IDictionary to set the value via indexer (to make sure that for duplicated keys, we use the last one)
        }
    ...
    public void Serialize(ref JsonWriter<TSymbol> writer, TDictionary? value)
    {
        ...
        var valueLength =
            CountFunctor(value); // IReadOnlyDictionary and IDictionary don't share the same interface with .Count, use expression trees to optimize it
